...
public class DBResult {
     ...
     * Метод вызывается при необходимости полной перезагрузки страницы
    ...
    /**
     * Возвращает строку для загрузки части контета страницы с результатом 
     * обработки. Метод вызывается при использовании пользователем кнопок 
     * истории перехода в браузере для загрузки только части страницы.
     * @param model Параметры jsp-страницы
     * @return Путь к странице с результатом обработки
     */
    public String doTile(Model model) { 
        return "page.body.content." + this.doSubmit(model);
    }
